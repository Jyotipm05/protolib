/**
 * @file UDP.hpp
 * @brief UDP client socket wrapper for IPv4 datagram communication.
 */
...
struct UdpEndpoint {
    /** @brief Dotted-decimal IPv4 address (for example, 127.0.0.1). */
    std::string address;
    /** @brief UDP port in host byte order. */
    std::uint16_t port{};
};

/** @brief Minimal UDP client API backed by Winsock2. */
class UDP {
public:
    /** @brief Constructs an empty UDP client with no open socket. */
    UDP() = default;
